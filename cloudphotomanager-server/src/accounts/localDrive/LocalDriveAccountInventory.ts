...
export async function LocalAccountInventoryGetFolder(
  ...
  return LocalAccountInventoryGetFolderByPath(context, localAccount, folder.folderpath);
}
...
export async function LocalAccountInventoryGetFolderByPath(
  ...
  folder.idCloud = path.join(localAccount.getAccountDefinition().rootpath, folderpath);
...
export async function LocalAccountInventoryListFoldersInFolder(
  ...
  for (const item of items) {
    ...
    if (stats.isDirectory()) {
      folders.push(
        await LocalAccountInventoryGetFolderByPath(context, localAccount, path.join(folder.folderpath, item))
      );
    }
  }
  return folders;
...
export async function LocalAccountInventoryListFilesInFolder(
  ...
  for (const item of items) {
    ...
    if (!stats.isDirectory()) {
      const stats = await stat(path.join(folder.idCloud, item));
      const file = new File(localAccount.getAccountDefinition().id, folder.id, item);
